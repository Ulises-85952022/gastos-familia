...
function getUpcomingDateInfo(p) {
  const now = new Date();
  const today = now.getDate();
  const year = now.getFullYear();
  const month = now.getMonth();
  let dueDate = new Date(year, month, p.dueDay);
  if (p.dueDay < today && !p.paid) dueDate = new Date(year, month + 1, p.dueDay);
  const todayDate = new Date(year, month, today);
  const diff = Math.round((dueDate - todayDate) / 86400000);
  const monthName = dueDate.toLocaleString('es-MX', { month: 'short' });
  return {
    date: p.dueDay + ' ' + monthName,
    daysLeft: diff,
    isToday: diff === 0,
    isOverdue: diff < 0,
  };
